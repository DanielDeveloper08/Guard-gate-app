...
export class AddVisitQrComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.startDateValue = this.formatDate(
      this.visitForm.get('startDate')?.value
    );
    this.endDateValue = this.calcularFechaFin();
  }
  ...
  calcularFechaFin(): string {
    const fechaInicio = this.startDateSameFormat
      ? new Date(this.startDateSameFormat)
      : new Date();
    const horasValidas = this.visitForm.get('validityHours')?.value;
    const fechaFin = new Date(
      fechaInicio.getTime() + (horasValidas || 0) * 60 * 60 * 1000
    );
  ...
  saveVisitQR() {
    ...
    if (visitData.validityHours == 0) {
      this._toastService.showInfo(
        'Por favor seleccione horas de validez',
        Position.Top
      );
      return;
    }

    // Validar si la hora de inicio es menor que la hora actual
    ...
    this._visitService.saveVisit(visitData).subscribe({
      next: (res) => {
        ...
        this._router.navigate([
          '/guard-gate/tabs/visit/success-qr',
          {
      ...
      error: (err: HttpErrorResponse) => {
        ...
        this._toastService.showError(
          'Ocurrió un error al generar la visita, comunicate con el administardor',
          Position.Top
        );
      },
    });
